...
//			yield return new WaitUntil(() => Input.GetButton("Jump"));
			...
			float force = GetForceFromTensorFlow(dist)*Random.Range(0.64f,0.68f);
	...
	void MoveRandom()
	{
        var dx = Random.Range(-0.5f, 0.5f);
        var dz = Random.Range(-0.5f, 0.5f);
        var newpos = transform.parent.position + new Vector3(dx, 0, dz);
        transform.parent.position = newpos;
        //var newPosition = new Vector3(TransformGoal.position.x + Random.Range(-0.1f, 0.1f), transform.parent.position.y, TransformGoal.position.z);
